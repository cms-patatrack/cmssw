...
int main() {
  ...
  for (int nav = 30; nav < 80; nav += 20) {
    ...
    for (int i = 8; i < 20; ++i) {
...
#ifdef __CUDACC__
      cudaMemcpy(LOC_WS(ntrks), &nt, sizeof(uint32_t), cudaMemcpyHostToDevice);
      cudaMemcpy(LOC_WS(zt), ev.ztrack.data(), sizeof(float) * ev.ztrack.size(), cudaMemcpyHostToDevice);
      cudaMemcpy(LOC_WS(ezt2), ev.eztrack.data(), sizeof(float) * ev.eztrack.size(), cudaMemcpyHostToDevice);
      cudaMemcpy(LOC_WS(ptt2), ev.pttrack.data(), sizeof(float) * ev.eztrack.size(), cudaMemcpyHostToDevice);
#else
...
#ifdef __CUDACC__
      cudaMemcpy(nn, LOC_ONGPU(ndof), nv * sizeof(int32_t), cudaMemcpyDeviceToHost);
      cudaMemcpy(chi2, LOC_ONGPU(chi2), nv * sizeof(float), cudaMemcpyDeviceToHost);
#else
      ...
      cudaMemcpy(&nv, LOC_WS(nvIntermediate), sizeof(uint32_t), cudaMemcpyDeviceToHost);
#else
      gridDim.x = 1024;  // nv ????
      assert(blockIdx.x == 0);
...
#ifdef __CUDACC__
      cudaMemcpy(zv, LOC_ONGPU(zv), nv * sizeof(float), cudaMemcpyDeviceToHost);
      cudaMemcpy(wv, LOC_ONGPU(wv), nv * sizeof(float), cudaMemcpyDeviceToHost);
      cudaMemcpy(chi2, LOC_ONGPU(chi2), nv * sizeof(float), cudaMemcpyDeviceToHost);
      cudaMemcpy(ptv2, LOC_ONGPU(ptv2), nv * sizeof(float), cudaMemcpyDeviceToHost);
      cudaMemcpy(nn , LOC_ONGPU(ndof), nv * sizeof(int32_t), cudaMemcpyDeviceToHost);
      cudaMemcpy(ind ,LOC_ONGPU(sortInd), nv * sizeof(uint16_t), cudaMemcpyDeviceToHost); 
#endif
      for (auto j = 0U; j < nv; ++j)
        if (nn[j] > 0)
          chi2[j] /= float(nn[j]);
