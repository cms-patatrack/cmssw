...
namespace  pixelProducerHelpers {
 ...
 inline 
 TrackingRecHit2DSOAView const * storeSoA(edm::Event& iEvent, std::unique_ptr<TrackingRecHit2DHost> & input, edm::EDPutTokenT<TrackingRecHit2DHost> const & outToken) {
   assert(input);
 ...
 inline
 uint32_t const * StoreModuleStart(edm::Event& iEvent, cms::cuda::host::unique_ptr<uint32_t[]> & input, edm::EDPutTokenT<HMSstorage> const & outToken) {
 ...
 inline
 uint32_t const * buildAndStoreModuleStart(edm::Event& iEvent, TrackerGeometry const & geom, const edmNew::DetSetVector<SiPixelCluster>& input, edm::EDPutTokenT<HMSstorage> const & outToken) {
     // fill cluster arrays
    int numberOfDetUnits = 0;
    int numberOfClusters = 0;
    auto hmsp = std::make_unique<uint32_t[]>(gpuClustering::MaxNumModules + 1);  // is the memory zeroed? (not required if clusInModule is used)
