...
cudautils::device::unique_ptr<float[]> TestCUDAProducerGPUKernel::runAlgo(const std::string &label,
  ...
  if (d_input != nullptr) {
    ...
    cudaMemcpy(h_check.get(), d_input, NUM_VALUES* sizeof(float), cudaMemcpyDeviceToHost);
  ...
  cudaMemcpyAsync(d_a.get(), h_a.get(), NUM_VALUES * sizeof(float), cudaMemcpyHostToDevice, stream);
  cudaMemcpyAsync(d_b.get(), h_b.get(), NUM_VALUES * sizeof(float), cudaMemcpyHostToDevice, stream);
