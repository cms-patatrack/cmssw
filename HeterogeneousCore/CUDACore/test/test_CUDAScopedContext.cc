...
namespace {
  std::unique_ptr<CUDAProduct<int*>> produce(int device, int* d, int* h) {
    ...
    cudaMemcpyAsync(d, h, sizeof(int), cudaMemcpyHostToDevice, ctx.stream());
...
TEST_CASE("Use of CUDAScopedContext", "[CUDACore]") {
  ...
  constexpr int defaultDevice = 0;
  {
    ...
    SECTION("Joining multiple CUDA streams") {
      ...
      cudaMemcpyAsync(&h_a1, d_a1.get(), sizeof(int), cudaMemcpyDeviceToHost, ctx.stream());
      cudaMemcpyAsync(&h_a2, d_a2.get(), sizeof(int), cudaMemcpyDeviceToHost, ctx.stream());
      cudaMemcpyAsync(&h_a3, d_a3.get(), sizeof(int), cudaMemcpyDeviceToHost, ctx.stream());
