...
private:
  ...
  std::map<std::pair<size_t, EncodedEventId>, TrackingParticleRef> 
  makeMap(const edm::Event &iEvent) const;
...
std::map<std::pair<size_t, EncodedEventId>, TrackingParticleRef>
ClusterTPAssociationProducerCUDA::makeMap(const edm::Event &iEvent) const {
  // TrackingParticle
...
void ClusterTPAssociationProducerCUDA::produce(edm::StreamID streamID, edm::Event& iEvent, const edm::EventSetup& iSetup) const {
  ...
  ctx.emplace(iEvent, tokenGPUProd_, m_gpuAlgo.makeAsync(gDigis, ndigis, gHits, digi2tp.data(), nhits, digi2tp.size(),ctx.stream()));
